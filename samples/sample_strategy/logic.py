    ...
    def handle_market_data(self):
        ...
        if self.current_position is None and self.entry_signal():
            ...
            trade_instructions.extend([
                TradeInstruction(self.AAPL, Action.LONG, self.trade_id, 1, self.asset_allocation()['AAPL']),
                TradeInstruction(self.MSFT, Action.SHORT, self.trade_id, 2, self.asset_allocation()['MSFT'])
            ])

        elif self.current_position and self.exit_signal():
            self.current_position = None
            trade_instructions.extend([
                TradeInstruction(self.AAPL, Action.SELL, self.trade_id, 1, 1.0),
                TradeInstruction(self.MSFT, Action.COVER, self.trade_id, 2, 1.0)
            ])
