...
class Cointegrationzscore(BaseStrategy):
    def __init__(self, symbols_map:Dict[str, Symbol], historical_data:pd.DataFrame, portfolio_server:PortfolioServer, logger:logging.Logger, order_book:OrderBook, event_queue:Queue):
    ...
    def prepare(self, historical_data:pd.DataFrame) -> None:
        ...
        self.train_data = self._log_prices(historical_data)
    ...
    def _cointegration(self, train_data:pd.DataFrame) -> list:
        ...
        # Log Results
        self.logger.info(TimeseriesTests.display_johansen_results(johansen_results, num_cointegrations, lag, False))
    ...
    def handle_market_data(self):
        ...
        if self._exit_signal(self.current_zscore, self.exit_threshold):
            ...
            trade_instructions = self.generate_trade_instructions(self.last_signal, trade_capital) 
            self.trade_id += 1
