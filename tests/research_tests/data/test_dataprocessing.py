...
class TestDataProcessing(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # Mock data for data retrieval and processing
        self.tickers = ['HE.n.0', 'ZC.n.0']
        self.start_date = '2022-05-01'
        self.end_date = '2022-05-03'

        # Mock database response of raw data
        self.db_response = [{"id":49252,"timestamp":"1651500000000000000","symbol":"HE.n.0","open":"104.0250","close":"103.9250","high":"104.2500","low":"102.9500","volume":3553},
        ...
        # Process mock database response data 
        self.valid_processed_data = valid_process_data(self.db_response)
    
    # Basic Validation
    def test_process_bardata_valid(self):
        df = pd.DataFrame(self.db_response)
        df.drop(columns=['id'], inplace=True)
    ...
    def test_get_data_valid(self):
        # Mock database response to api call
        self.mock_db.get_bar_data.return_value = self.db_response 

        # Test
        result = self.data_processor.get_data(tickers=self.tickers, 
                                  start_date=self.start_date, 
                                  end_date=self.end_date)

        # Validate 
        self.mock_db.get_bar_data.assert_called_once_with(tickers=self.tickers, start_date=self.start_date, end_date=self.end_date)
        assert_frame_equal(result, self.valid_processed_data, check_dtype=True)
    ...
    def test_get_data_invalid_missing_values_strategy(self):
        with self.assertRaisesRegex(ValueError, "'missing_value_strategy' must either 'fill_forward' or 'drop' of type str."):
            self.data_processor.get_data(tickers=self.tickers,
                            start_date=self.start_date, 
                            end_date=self.end_date,
