...
from midas.shared.symbol import Symbol, Industry, Equity, Currency, Venue, SecurityType 

DATABASE_KEY: str = config("LOCAL_API_KEY")
DATABASE_URL: str = config("LOCAL_URL")
...
class TestBarDataMethods(unittest.TestCase):
    ...
    def tearDownClass(cls) -> None:
        ...
        resources = [
            (
                "asset classes",
                cls.admin_client.get_asset_classes,
                cls.admin_client.delete_asset_class,
            ),
            (
                "security types",
                cls.admin_client.get_security_types,
                cls.admin_client.delete_security_type,
            ),
            ("venues", cls.admin_client.get_venues, cls.admin_client.delete_venue),
            (
                "currencies",
                cls.admin_client.get_currencies,
                cls.admin_client.delete_currency,
            ),
            (
                "industries",
                cls.admin_client.get_industries,
                cls.admin_client.delete_industry,
            ),
            (
                "contract units",
                cls.admin_client.get_contract_units,
                cls.admin_client.delete_contract_units,
            ),
        ]

        for _, getter, deleter in resources:
            try:
                items = getter()
                for item in items:
                    try:
        ...
        def delete_symbol(symbol: Symbol):
            try:
                ...
                if len(response) == 0:
                    logging.debug(
                        f"No symbol found for ticker {symbol.ticker}, nothing to delete."
                    )
                elif len(response) == 1:
                    ...
                    if len(verification) == 0:
                        logging.debug(
                            f"Successfully deleted symbol with ticker {symbol.ticker}."
                        )
                    else:
                        logging.error(
                            f"Failed to delete symbol with ticker {symbol.ticker}, it still exists."
                        )
                else:
                    logging.error(
                        f"Multiple entries found for ticker {symbol.ticker}, not deleting."
                    )
...
class TestTradingSessioMethods(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.session_obj = LiveTradingSession(
            parameters=self.parameters,
