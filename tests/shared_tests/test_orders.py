...
from midas.shared.orders import Action, OrderType, MarketOrder, LimitOrder, StopLoss
...
class TestMarketOrder(unittest.TestCase):
    ...
    def test_valid_construction(self):
        ...
        self.assertEqual(base_order.order.action, self.action.to_broker_standard())
    ...
    def test_type_check(self):
        with self.assertRaisesRegex(TypeError, "'action' field must be type Action enum."):
            MarketOrder(action='self.action,', quantity=self.quantity)
            
        with self.assertRaisesRegex(TypeError, "'quantity' field must be type float or int."):
            MarketOrder(action=self.action, quantity="self.quantity")
    
    # Constraint Check
    def test_value_constraint(self):
        with self.assertRaisesRegex(ValueError,"'quantity' field must not be zero."):
...
class TestLimitOrder(unittest.TestCase):
    ...
    def test_valid_construction(self):
        # Test
        base_order = LimitOrder(action=self.action,
    ...
    def test_type_checks(self):
        ...
        with self.assertRaisesRegex(TypeError,"'limit_price' field must be of type float or int."):
            LimitOrder(action=self.action,
                        quantity=self.quantity,
                        limit_price="self.limit_price")
    ...
    def test_value_constraint(self):
        ...
        with self.assertRaisesRegex(ValueError, "'limit_price' field must be greater than zero."):
            LimitOrder(action=self.action,
                        quantity=self.quantity, 
                        limit_price=0.0)
...
class TestStopLoss(unittest.TestCase):
    ...
    def test_valid_construction(self):
        # Test
        base_order = StopLoss(action=self.action,
                               quantity=self.quantity,
    ...
    def test_type_checks(self):
        ...
        with self.assertRaisesRegex(TypeError,"'aux_price' field must be of type float or int."):
            StopLoss(action=self.action,
                        quantity=self.quantity,
                        aux_price="self.aux_price")
    ...
    def test_value_constraint(self):
        ...
        with self.assertRaisesRegex(ValueError, "'aux_price' field must be greater than zero."):
            StopLoss(action=self.action,
                        quantity=self.quantity, 
                        aux_price=0.0)
