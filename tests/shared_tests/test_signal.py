...
class TestTradeInsructions(unittest.TestCase):
    ...
    def test_construction_market_order(self):
        # Test
        instructions = TradeInstruction(ticker=self.ticker,
        ...
        # Validate
        self.assertEqual(instructions.ticker, self.ticker)
    ...
    def test_market_to_dict(self):
        ...
        # Validate
        self.assertEqual(instructions_dict['ticker'], self.ticker)
        self.assertEqual(instructions_dict['order_type'], self.order_type.value)
    ...
    def test_limit_to_dict(self):
        ...
        # Validate
        self.assertEqual(instructions_dict['ticker'], self.ticker)
        self.assertEqual(instructions_dict['order_type'], OrderType.LIMIT.value)
        self.assertEqual(instructions_dict['action'], self.action.value)
        self.assertEqual(instructions_dict['trade_id'], self.trade_id)
        self.assertEqual(instructions_dict['leg_id'], self.leg_id)
        self.assertAlmostEqual(instructions_dict['weight'], self.weight, 4)
    ...
    def test_stop_loss_to_dict(self):
        ...
        # Validate
        self.assertEqual(instructions_dict['ticker'], self.ticker)
        self.assertEqual(instructions_dict['order_type'], OrderType.STOPLOSS.value)
        self.assertEqual(instructions_dict['action'], self.action.value)
        self.assertEqual(instructions_dict['trade_id'], self.trade_id)
        self.assertEqual(instructions_dict['leg_id'], self.leg_id)
        self.assertAlmostEqual(instructions_dict['weight'], self.weight, 4)
    ...
    def test_type_checks(self):
        with self.assertRaisesRegex(TypeError, "'ticker' field must be of type string."):
        ...
        with self.assertRaisesRegex(TypeError, "'order_type' field must be of type OrderType enum."):
            TradeInstruction(ticker=self.ticker,
                                order_type='MKT',
                                action=self.action,
                                trade_id=self.trade_id,
                                leg_id=self.leg_id,
        ...
        with self.assertRaisesRegex(TypeError, "'trade_id' field must of type int."):
            TradeInstruction(ticker=self.ticker,
                                order_type=self.order_type,
                                action=self.action,
                                trade_id='2',
        ...
        with self.assertRaisesRegex(TypeError, "'limit_price' field must be int or float for OrderType.LIMIT."):
            TradeInstruction(ticker=self.ticker,
                                order_type=OrderType.LIMIT,
                                action=self.action,
                                trade_id=self.trade_id,
        ...
        with self.assertRaisesRegex(TypeError, "'aux_price' field must be int or float for OrderType.STOPLOSS."):
            TradeInstruction(ticker=self.ticker,
                                order_type=OrderType.STOPLOSS,
                                action=self.action,
                                trade_id=self.trade_id,
    ...
    def test_value_constraint(self):            
        ...
        with self.assertRaisesRegex(ValueError, "'leg_id' field must must be greater than zero."):
            TradeInstruction(ticker=self.ticker,
                                order_type=self.order_type,
                                action=self.action,
                                trade_id=self.trade_id,
        ...
        with self.assertRaisesRegex(ValueError, "'limit_price' field must must be greater than zero."):
            TradeInstruction(ticker=self.ticker,
                                order_type=OrderType.LIMIT,
                                action=self.action,
                                trade_id=self.trade_id,
        ...
        with self.assertRaisesRegex(ValueError, "'aux_price' field must must be greater than zero."):
            TradeInstruction(ticker=self.ticker,
                                order_type=OrderType.STOPLOSS,
                                action=self.action,
                                trade_id=self.trade_id,
