...
class Symbol(ABC):
    ...
    def __post_init__(self):
        ...
        if self.data_ticker is not None and not isinstance(
            self.data_ticker, str
        ):
...
class SymbolMap:
    def __init__(self, map: Dict[str, Symbol] = None):
        """
        Initializes the SymbolMap with an optional initial map of symbols.

        Parameters:
        - symbols_map (Dict[str, Symbol], optional): A dictionary of symbols to initialize the map.
        """
        # If no initial map is provided, start with an empty dictionary
        self.map = map if map is not None else {}

    def add_symbol(self, key: str, symbol: Symbol) -> None:
        """
        Add the symbol to the map if it doesn't already exist.

        Parameters:
        - key (str): The key to use in the symbols map (e.g., ticker symbol).
