...
from midas.engine.observer import Observer, Subject, EventType
from midas.engine.risk import BaseRiskModel
...
class RiskHandler(Observer, Subject):
    def __init__(self, risk_model_class: Type[BaseRiskModel]):
        ...
        """
        if not risk_model_class or not issubclass(risk_model_class, BaseRiskModel):
            raise TypeError(f"Risk model wrong type.")
        
        # Initialize subject part
        Subject.__init__(self)
        self.risk_model = risk_model_class()

    def update(self, subject: Subject, event_type: EventType, data: dict = None):
        """
