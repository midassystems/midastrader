...
from midas.shared.symbol import SecurityType, Symbol, Right
...
class Position(ABC):
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.price_multiplier, (int, float)):
            raise TypeError("'price_multiplier' field must be of type int or float.")
        ...
        if not isinstance(self.market_price, (int, float)):
            raise TypeError("'market_price' field must be of type int or float.")

        # Value constraints
        if self.action not in ['BUY', 'SELL']:
            raise ValueError("'action' field must be either ['BUY','SELL'].")
        if self.price_multiplier <= 0:
            raise ValueError("'price_multiplier' field must be greater than zero.")
        if self.quantity_multiplier <= 0:
            raise ValueError("'quantity_multiplier' field must be greater than zero.")
...
class FuturePosition(Position):
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.initial_margin, (int, float)):
            raise TypeError("'initial_margin' field must be of type int or float.")
    ...
    def calculate_initial_value(self) -> None:
        self.initial_value = self.avg_price * self.price_multiplier * self.quantity * self.quantity_multiplier

    def calculate_initial_cost(self) -> None:
        self.initial_cost = self.initial_margin * abs(self.quantity)

    def calculate_market_value(self) -> None:
        self.market_value = self.market_price * self.price_multiplier * self.quantity * self.quantity_multiplier

    def calculate_unrealized_pnl(self) -> None:
        self.unrealized_pnl = (self.market_price - self.avg_price) * self.price_multiplier * self.quantity * self.quantity_multiplier

    def calculate_margin_required(self) -> None:
        self.margin_required =  self.initial_margin * abs(self.quantity)

    def calculate_liquidation_value(self) -> None:
        self.liquidation_value = self.initial_cost + ((self.market_price - self.avg_price) * self.price_multiplier * self.quantity * self.quantity_multiplier)
    ...
    def update(self, quantity: int, price: float, action: str) -> Impact:
        ...
        if action == self.action: # Adding to the same position
            new_avg_price = ((self.avg_price * self.quantity) + (price * quantity)) / new_quantity
        ...
        remaining_unrealized_pnl = initial_market_value_after_trade - initial_value_after_trade
    ...
    def pretty_print(self) -> str:
        ...
        string += f"  Initial Margin': {self.initial_margin}\n"
        return string
...
class EquityPosition(Position):
    ...
    def calculate_initial_value(self) -> None:
        self.initial_value = self.avg_price * self.quantity * self.quantity_multiplier

    def calculate_initial_cost(self) -> None:
        self.initial_cost = self.initial_value

    def calculate_market_value(self) -> None:
        self.market_value = self.market_price * self.quantity * self.quantity_multiplier

    def calculate_unrealized_pnl(self) -> None:
        self.unrealized_pnl = (self.market_price * self.quantity * self.quantity_multiplier) - self.initial_cost
    
    def calculate_margin_required(self) -> None:
        self.margin_required = 0
    
    def calculate_liquidation_value(self) -> None:
        self.liquidation_value = self.market_price * self.quantity * self.quantity_multiplier
...
class OptionPosition(Position):
    ...
    def calculate_initial_value(self) -> None:
        self.initial_value = self.avg_price * self.quantity_multiplier  * self.price_multiplier * self.quantity

    def calculate_initial_cost(self) -> None:
        initial_cost = self.avg_price * self.quantity_multiplier  * self.price_multiplier * self.quantity
    ...
    def calculate_unrealized_pnl(self) -> None:
        if self.action == 'BUY':
            ...
            self.unrealized_pnl = (self.avg_price - self.market_price) * self.price_multiplier * self.quantity * self.quantity_multiplier
        else:
    ...
    def calculate_liquidation_value(self) -> None:
        self.liquidation_value = self.market_price * self.price_multiplier * self.quantity * self.quantity_multiplier
    ...
    def pretty_print(self) -> str:
        string = super().pretty_print()
        string += f"  Strike Price: {self.strike_price}\n"
        string += f"  Expiration date: {self.expiration_date}\n"
        string += f"  Type: {self.type.value}\n"
        return string
    

def position_factory(asset_type: SecurityType, symbol: Symbol, **kwargs) -> Position:
