...
class Backtest:
    ...
    def __post_init__(self):
        ...
        if not all(isinstance(item, dict) for item in self.daily_timeseries_stats):
            raise ValueError("'daily_timeseries_stats' field must be a list of dictionaries.")
        if not all(isinstance(item, dict) for item in self.period_timeseries_stats):
            raise ValueError("'period_timeseries_stats' field must be a list of dictionaries.")
